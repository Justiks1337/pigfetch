...
impl SystemInfo {
    ...
    fn get_username() -> Option<String> {
        var_os("USER").and_then(|os_string: OsString| os_string.into_string().ok())
    }

    fn get_os() -> Option<String> {
        let osinfo = os_info::get();
        if Some(&osinfo).is_some() {
            return Some(format!("{} {}", osinfo.os_type(), osinfo.version()));
        }
        None
    }

    fn get_host() -> Option<String> {
        get().ok().and_then(|result| result.into_string().ok())
    }
    ...
    fn get_desktop() -> Option<String> {
        var_os("XDG_SESSION_DESKTOP").and_then(|os_string: OsString| os_string.into_string().ok())
    }
    ...
    fn get_package_managers() -> Option<String> {
        None
    }

    fn get_shell() -> Option<String> {
        var_os("SHELL").and_then(|os_string: OsString| os_string.into_string().ok())
    }

    fn get_de() -> Option<String> {
        var_os("DESKTOP_SESSION").and_then(|os_string: OsString| os_string.into_string().ok())
    }
    ...
    fn get_memory() -> Option<String> {
        if let Ok(info) = mem_info() {
            ...
            return Some(format!("{}Gi/{}Gi", available_memory, total));
        }
        None
    }
