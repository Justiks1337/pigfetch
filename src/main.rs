...
fn main() {
    let mut artfile = arts_file::ArtsFile::new();
    artfile.read_file();
    ...
    let binding = system_info::SystemInfo::new();
    let mut system_info = binding.to_hashmap().into_iter();
    println!("\n");
    for line in art {
        let (attr, value_option) = loop {
        ...
        println!(
            "    \x1B[35m\x1B[1m{} \x1B[0m \x1B[35m\x1B[1m{}:\x1B[0m {}",
            line,
            attr,
            value.clone()
        );
